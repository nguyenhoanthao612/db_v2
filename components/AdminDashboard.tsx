                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/60 space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] text-slate-500 uppercase font-black tracking-wider">Danh sách các Phát biểu / Hàng</span>
                          <button
                            type="button"
                            onClick={() => setMatrixRows([...matrixRows, ''])}
                            className="px-2.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-[10px] font-bold transition flex items-center gap-1 cursor-pointer shadow-sm"
                          >
                        ...
                        <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
                          {matrixRows.map((row, rIdx) => (
                            <div key={rIdx} className="flex gap-2 items-center">
                              <span className="flex items-center justify-center w-5 h-5 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-black flex-shrink-0">
                                {rIdx + 1}
                              </span>
                              <input
                                type="text"
                                required
                                placeholder={`Nhập phát biểu ${rIdx + 1}...`}
                                value={row}
                                onChange={(e) => {
                                  ...
                                  // Preserve correct answer choice mapping if renamed
                                  if (matrixCorrect[row]) {
                                    const updatedCorrect = { ...matrixCorrect };
                                    const colVal = updatedCorrect[row];
                                    delete updatedCorrect[row];
                                    updatedCorrect[newVal] = colVal;
                                    setMatrixCorrect(updatedCorrect);
                                  }
                                }}
                                className="flex-1 px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20"
                              />
                              {matrixRows.length > 1 && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    const newRows = matrixRows.filter((_, i) => i !== rIdx);
                                    setMatrixRows(newRows);
                                    const updatedCorrect = { ...matrixCorrect };
                                    delete updatedCorrect[row];
                                    setMatrixCorrect(updatedCorrect);
                      ...
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/60 space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] text-slate-500 uppercase font-black tracking-wider">Danh sách các Phương án / Cột</span>
                          <button
                            type="button"
                            onClick={() => setMatrixCols([...matrixCols, ''])}
                            className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[10px] font-bold transition flex items-center gap-1 cursor-pointer shadow-sm"
                          >
                        ...
                        <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
                          {matrixCols.map((col, cIdx) => (
                            <div key={cIdx} className="flex gap-2 items-center">
                              <span className="flex items-center justify-center w-5 h-5 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black flex-shrink-0">
                                {cIdx + 1}
                              </span>
                              <input
                                type="text"
                                required
                                placeholder={`Nhập phương án ${cIdx + 1}...`}
                                value={col}
                                onChange={(e) => {
                                  const newVal = e.target.value;
                                  const oldVal = matrixCols[cIdx];
                                  const newCols = [...matrixCols];
                                  newCols[cIdx] = newVal;
                                  setMatrixCols(newCols);
                                  // Update mapping in matrixCorrect
                                  const updatedCorrect = { ...matrixCorrect };
                                  Object.keys(updatedCorrect).forEach((rowKey) => {
                                    if (updatedCorrect[rowKey] === oldVal) {
                                      updatedCorrect[rowKey] = newVal;
                                    }
                                  });
                                  setMatrixCorrect(updatedCorrect);
                                }}
                              ...
                              {matrixCols.length > 1 && (
                                <button
                                  ...
                                  onClick={() => {
                                    const oldVal = matrixCols[cIdx];
                                    const newCols = matrixCols.filter((_, i) => i !== cIdx);
                                    setMatrixCols(newCols);
                                    const updatedCorrect = { ...matrixCorrect };
                                    Object.keys(updatedCorrect).forEach((rowKey) => {
                                      if (updatedCorrect[rowKey] === oldVal) {
                                        delete updatedCorrect[rowKey];
                                      }
                                    });
                                    setMatrixCorrect(updatedCorrect);
                                  }}
